...
char zero[2]= "0";char nptr[256];
char *intToStr(int value){int m=1;char *text;
  while(((float)value)>=pow(10,m))m++;
...
#define xinit  x.cnt = 0; x.len = 0; x.data = (char**)malloc(1048576*4); 
...
int test()
{
   xinit; stm
   line
   int i,fd = open("/sdcard/Download/uiUX.html", O_RDONLY);
 ...
 enc=lzw_encode(in,15);
  c("lzw size: ")m(_len(enc)) line
...
extern "C"
{
	JNIEXPORT jstring JNICALL Java_com_twosine_zsx_Client_serveHTTP(JNIEnv* env, jobject thiz)
	{
		serve();
		return env->NewStringUTF("served.");
	}
}
